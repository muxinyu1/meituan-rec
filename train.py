...
# 整合所有连续特征，方便归一化
ALL_CONTINUOUS_FEATURES = USER_CONTINUOUS_FEATURES + ITEM_CONTINUOUS_FEATURES + CONTEXT_CONTINUOUS_FEATURES
...
class RecSysDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 构建上下文特征字典
        context_features = {name: sample_row[name] for name in self.context_discrete_cols}
...
def collate_batch(batch):
    ...
    # 初始化批处理字典
    for name in USER_DISCRETE_VOCAB_SIZES: batched_user[name] = []
    for name in USER_CONTINUOUS_FEATURES: batched_user[name] = []
    for name in ITEM_DISCRETE_VOCAB_SIZES: batched_item[name] = []
    for name in ITEM_CONTINUOUS_FEATURES: batched_item[name] = []
    for name in CONTEXT_DISCRETE_VOCAB_SIZES: batched_context[name] = []
    for name in CONTEXT_CONTINUOUS_FEATURES: batched_context[name] = []
    
    for user_feats, item_feats, context_feats, label in batch:
        for name, val in user_feats.items(): batched_user[name].append(val)
        for name, val in item_feats.items(): batched_item[name].append(val)
        for name, val in context_feats.items(): batched_context[name].append(val)
        labels.append(label)
...
def evaluate(model, data_loader, device):
    ...
    with torch.no_grad():
        for user_batch, item_batch, context_batch, labels in tqdm(data_loader, desc="Validating"):
...
def main():
    ...
    # --- 划分训练集和验证集 ---
    print(f"Splitting data into train and validation sets ({1-VALIDATION_SPLIT:.0%}:{VALIDATION_SPLIT:.0%})")
    ...
    if ALL_CONTINUOUS_FEATURES:
        ...
        train_df.loc[:, ALL_CONTINUOUS_FEATURES] = scaler.fit_transform(train_df[ALL_CONTINUOUS_FEATURES])
        # Only transform on validation data
        val_df.loc[:, ALL_CONTINUOUS_FEATURES] = scaler.transform(val_df[ALL_CONTINUOUS_FEATURES])
    ...
    train_loader = DataLoader(
        ...
        collate_fn=collate_batch, num_workers=num_worker_cores, pin_memory=True
    )
    ...
    # --- 初始化模型、损失函数、优化器 ---
    model = Model(
        user_feature_defs=USER_FEATURE_DEFS, item_feature_defs=ITEM_FEATURE_DEFS,
        context_feature_defs=CONTEXT_FEATURE_DEFS,
        embedding_dim_per_feature=EMBEDDING_DIM_PER_FEATURE,
        hidden_dim=HIDDEN_DIM, emb_dim=FINAL_EMB_DIM, head_nums=HEAD_NUMS,
    ).to(device)
